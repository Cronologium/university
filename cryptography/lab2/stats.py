...
def main():
    ...
    for x in xrange(1, 4):
        ...
        for line in data.split('\n'):
            ...
            worksheet.write('D' + str(line_no), len(line.split(' ')[2]))
    ...
    chart.add_series({'name': 'modulo',     'values': '=Sheet1!$A$1:$A$%s' % str(size)})
    chart.add_series({'name': 'difference', 'values': '=Sheet1!$B$1:$B$%s' % str(size)})
    chart.add_series({'name': 'binary',     'values': '=Sheet1!$C$1:$C$%s' % str(size)})
